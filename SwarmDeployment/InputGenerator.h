...
public:
	...
	vector<unordered_map<Uav, shared_ptr<CarLikeControl>, UavHasher>> generateAllInputs(int distance_of_new_nodes, double max_turn, vector<shared_ptr<Uav>> uavs);

protected:
	NTupletGenerator generator;
	vector<shared_ptr<CarLikeControl>> generateOneUavInputs(int distance_of_new_nodes, double max_turn);
	...
	map<string, vector<unordered_map<Uav, shared_ptr<CarLikeControl>, UavHasher>>> cache;	//for same inputs are always same outputs. There will be stored inputs and their outputs.
