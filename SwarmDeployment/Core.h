...
namespace App
{
	...
	public:
		...
		vector<shared_ptr<State>> runRRTPath();
		...
		shared_ptr<LinkedState> nearest_neighbor(unordered_map<UavForRRT, shared_ptr<Point>, UavHasher> s_rand, vector<shared_ptr<LinkedState>> nodes, int count);
		shared_ptr<LinkedState> select_input(unordered_map<UavForRRT, shared_ptr<Point>, UavHasher> s_rand, shared_ptr<LinkedState> near_node, shared_ptr<Map> map, std::map<string, shared_ptr<Node>> mapNodes);
		...
		void check_near_goal(vector<shared_ptr<UavForRRT>> uavs, shared_ptr<Map> map); // vrací pole dlouhé tak, jako je poèet UAV. pro každé UAv se tak uloží do tohoto pole èíslo podle toho, v kolikátém cíli UAV je. Parametr je pole, kde jsou polohy UAV
		...
		shared_ptr<LinkedState> carLikeMotionModel(shared_ptr<LinkedState> state, unordered_map<UavForRRT, CarLikeControl, UavHasher> inputs);
