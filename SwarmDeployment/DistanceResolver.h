...
namespace App
{
	...
	public:
		...
		double getDistance(shared_ptr<StateInterface> first, unordered_map<UavForRRT, shared_ptr<Point>, UavHasher> second);
		double getDistance(shared_ptr<StateInterface> first, shared_ptr<StateInterface> second);
		double getDistance(shared_ptr<StateInterface> first, shared_ptr<StateInterface> second, shared_ptr<UavForRRT> uav);
		...
		double getLengthOfPath(vector<shared_ptr<State>> path, shared_ptr<UavForRRT> uav);
