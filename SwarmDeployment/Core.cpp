...
//			%Random state
			vector<shared_ptr<Point>> s_rand = random_state_guided(guiding_paths, map, nearState); // vrátí pole náhodnıch bodù, jeden pro kadou kvadrokoptéru
			...
			while (!near_found)
			{
				...
				nodes.push_back(nearState); // promìnná nodes je pole, kam se ukládá strom prohledávání u RRT - Path. Nemìlo by bıt potøeba tohle pøiøazovat, protoe tam je reference, ne hodnota
	...
	vector<shared_ptr<Point>> Core::random_state_guided(vector<shared_ptr<Path>> guiding_paths, shared_ptr<Map> map, shared_ptr<State> state)
	{
		...
		if (random > guided_sampling_prob) //vybírá se náhodnı vzorek
		{
			...
			for (size_t i = 0; i < state->uavs.size(); i++)
			{
				...
				{//todo: s tímhle nìco udìlat, a nepøistupovat k poli takhle teple pøes indexy
					randomStates[i] = random_state_goal(uav->getReachedGoal(), map);	//pokud je n-té UAV v cíli, vybere se náhodnı bod z cílové plochy, kam UAV dorazilo
				}
				else
				{
					randomStates[i] = random_state(0, 0, worldWidth, worldHeight, map); // pokud n-té UAV není v cíli, vybere se náhodnı bod z celé mapy
				}
			}
		...
		} else
		{
			...
			for (auto group : uavGroups)
			{
				...
				for (size_t i = 0; i < group->getUavs().size(); i++)	//todo: vymyslet, jak nìjak inteligentnì indexovat randomStates, abch nemusel pouívat tenhle teplı for cyklus
				{
...
//		// params.sampling_radius = defaults.sampling_radius;
	...
	shared_ptr<State> Core::car_like_motion_model(shared_ptr<State> node, vector<shared_ptr<Point>> inputs)
	{
		...
		for (size_t j = 0; j < number_of_uavs; j++)
		{
			...
			double dPhi = (inputs[j]->getX() / L) * tan(inputs[j]->getY());	//dPhi se nemìní v rámci vnitøního cyklu, take staèí spošítat jen jednou

			for (double i = time_step; i < end_time; i += time_step)
			{
				//calculate derivatives from inputs
				double dx = inputs[j]->getX() * cos(uavPointParticle->getRotation()->getZ());	//pokud jsme ve 2D, pak jediná moná rotace je rotace okolo osy Z
				double dy = inputs[j]->getX() * sin(uavPointParticle->getRotation()->getZ());	//input není klasickı bod se souøadnicemi X, Y, ale objekt se dvìma èísly, odpovídajícími dvìma vstupùm do car_like modelu
