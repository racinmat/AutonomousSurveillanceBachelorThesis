...
//		throw runtime_error("No valid input found.");
		...
		for (int ii = 0; ii < initialState->uavs.size(); ii++)	//promìnná ii, aby se pøi debuggingu nepletla s promìnnou i, která se pouívá ve velkém while cyklu
		{
			...
			{
				initialState->uavs[ii]->current_index[j] = guiding_paths[j]->getSize() - 1;	//protoe se indexuje od 0
			}
		}
...
//			%Random state
			vector<shared_ptr<Point>> s_rand = random_state_guided(guiding_paths, goals_reached, map, nearState); // vrátí pole náhodnıch bodù, jeden pro kadou kvadrokoptéru
