...
//		throw runtime_error("No valid input found.");
		...
		for (int ii = 0; ii < initialState->uavs.size(); ii++)	//promìnná ii, aby se pøi debuggingu nepletla s promìnnou i, která se pouívá ve velkém while cyklu
		{
	...
	vector<shared_ptr<Point>> Core::random_state_guided(vector<shared_ptr<Path>> guiding_paths, shared_ptr<Map> map, shared_ptr<State> state)
	{
		...
		if (random > guided_sampling_prob) //vybírá se náhodnı vzorek
		{
			for (size_t i = 0; i < number_of_uavs; i++)
			{
				...
				{//todo: s tímhle nìco udìlat, a nepøistupovat k poli takhle teple pøes indexy
					s_rand[i] = random_state_goal(state->uavs[i]->getReachedGoal(), map);	//pokud je n-té UAV v cíli, vybere se náhodnı bod z cílové plochy, kam UAV dorazilo
				} else
				{
		...
		} else
		{
			...
			for (size_t i = 0; i < uavGroups.size(); i++)
			{
				...
				{
					if (group->getUavs()[j]->isGoalReached())	//todo: zjistit, jestli tam nemá bıt index cíle
					{
