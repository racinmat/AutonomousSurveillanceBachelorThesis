	...
	shared_ptr<State> Core::car_like_motion_model(shared_ptr<State> node, vector<shared_ptr<Point>> inputs)
	{
		...
		for (double i = time_step; i < end_time; i += time_step)
		{
			...
			for (size_t j = 0; j < number_of_uavs; j++)
			{
				...
				double dx = inputs[j]->getX() * cos(newNode->uavs[j]->getPointParticle()->getRotation()->getZ());	//pokud jsme ve 2D, pak jediná moná rotace je rotace okolo osy Z
				double dy = inputs[j]->getX() * sin(newNode->uavs[j]->getPointParticle()->getRotation()->getZ());	//input není klasickı bod se souøadnicemi X, Y, ale objekt se dvìma èísly, odpovídajícími dvìma vstupùm do car_like modelu
