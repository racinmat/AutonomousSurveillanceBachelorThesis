...
//			%Random state
	...
	shared_ptr<LinkedState> Core::select_input(unordered_map<UavForRRT, shared_ptr<Point>, UavHasher> randomState, shared_ptr<LinkedState> nearState, shared_ptr<Map> map, std::map<string, shared_ptr<Node>> mapNodes)
	{
		...
		auto inputs = inputGenerator->generateAllInputs(distance_of_new_nodes, max_turn, nearState->getUavsForRRT());		//poèet všech kombinací je poèet všech monıch vstupù jednoho UAV ^ poèet UAV
	...
	vector<shared_ptr<UavGroup>> Core::splitUavsToGroups(vector<shared_ptr<Path>> guiding_paths, shared_ptr<Map> map, shared_ptr<LinkedState> state, bool allowSwarmSplitting)
	{
		...
		else
		{
			...
			else if (placementMethod == PlacementMethod::Chain)
			{
				...
				//pokud mám PlacementMethod::Chain, mám 3 skupiny. První UAV je jeden konec, poslední UAV je druhı konec, zbytek UAV nemají cíl a jsou 3. skupina
				//uavGroups.size == 2, protoe jsou 2 cesty. 
				...
				// protoe na konci nìkterá UAV musí skonèit mimo AoI, musím pro nì udìlat nìco, co se bude tváøit vdy jako cíl. Proto udìlám virtuální cíl velkı jako celá mapa
